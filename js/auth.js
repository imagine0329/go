...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        if (doc.exists) {
            ...
            // Check if approved (admin always approved)
            if (data.approved === false && data.role !== 'admin') {
                showPending();
                return;
            }
        } else {
...
function sendDesktopNotification(title, subtitle, body) {
    ...
    notif.onclick = function() {
        window.focus();
        switchTab('feed', document.querySelector('.sidebar-btn'));
        notif.close();
    };
}
function showLogin() {
...
async function handleRegister() {
    ...
    try {
        ...
        await db.collection('users').doc(cred.user.uid).set({
            email, name, role: 'member', approved: false, joinDate: today
        });
        showToast('📩 Sign up complete! Waiting for admin approval.');
    } catch (e) {
...
function switchTab(tab, btn) {
    ...
    if (tab === 'memorize') { attachCardsListener(); renderMemorize(); }
}
